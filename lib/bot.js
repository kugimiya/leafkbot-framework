...
class KBot {
  ...
  async switchBlock(blockId) {
    const blk = await this.bot.inventory.findInventoryItem(blockId, null);
    await this.bot.equip(blk, 'hand', () => {});
  }

  distanseToVec3(position) {
  ...
  async gotoBlock(refBlock) {
    ...
    const distanceToBlk = this.distanseToVec3(refBlock.position);

    if (distanceToBlk > this.MAX_TO_BLOCK_DISTANCE) {
      let nearestBlkPosValid = false;
      let nearestBlkPos;
      let distanceInPercents = 100 / (distanceToBlk / this.MAX_TO_BLOCK_DISTANCE);
      ...
      while (!nearestBlkPosValid) {
        ...
        if (!this.isAir(this.bot.blockAt(nearestBlkPos))) {
